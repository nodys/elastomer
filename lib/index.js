...
var createProto = require('./createproto')
...
function elastomer (name, init) {
  ...
  if (init.prototype) {
    try {
      ...
      console.warn('Unable to register this element %s. Try to export currently registered element (slow).', name, err.message)
      try {
  ...
  try {
    ...
    console.warn('Unable to set localName on %s', name, e)
  }
...
elastomer.binders = require('./binders')
elastomer.formatters = require('./formatters')
elastomer.createProto = createProto
