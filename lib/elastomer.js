...
var elastoshade = require('./elastoshade.js')
  ...
  if (this.useNative) {
    try {
      ...
      if (Elastomer.config.forceNative) {
        ...
        console.warn('Can not use native shadow as requested: disgrade to elastoshade')
        this.useNative = false
      }
      this.shadowRoot = this.host
    }
  } else {
    this.shadowRoot = elastoshade(this)
  }
}
...
Elastomer.prototype._injectHtml = function () {
  ...
  if (this.useNative) {
    ...
    this.shadowRoot.innerHTML = this.html.toString()
    this._bind(this.host) // Could be shadowRoot when API is complete
  }
}
...
Elastomer.prototype.mapAttribute = function (attribute, formatter, callback) {
  ...
  var scopeWatcher = self.watch(self.getScope(), formatter.path, function (value, previous) {
    if (formatter.way & MapWay.UP) {
      ...
      setTimeout(function () { attrWatcher.setValue(formatter.publish(value)) })
    }
  }, true)
...
Elastomer.prototype.mapProperty = function (property, formatter, callback) {
  ...
  scopeWatcher = self.watch(self.getScope(), formatter.path, function (value, previous) {
    if (formatter.way & MapWay.UP) {
      ...
      setTimeout(function () { propertyWatcher.setValue(formatter.publish(value)) })
    }
  }, true)
