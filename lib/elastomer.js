...
Elastomer.prototype.emit = function (obj, eventName, data) {
  ...
  var isDom   = obj.dispatchEvent ? true : false
  if(isDom) {
...
Elastomer.prototype.on = function (obj, eventName, handler) {
  ...
  var isDom   = obj.addEventListener ? true : false
  var res;

  this.addClosable(function() {
    res = isDom ? obj.addEventListener(eventName, handler) : obj.on(eventName, handler)
    return {
      close: function() {
        return isDom ? obj.removeEventListener(eventName, handler) : obj.off(eventName, handler)
      }
    }
  })
  return res
