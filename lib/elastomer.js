...
function Elastomer (host, init) {
  ...
  this.lightDom = []
  this.useNative = false // this.host.createShadowRoot ?
...
Elastomer.prototype.createdCallback = function () {
  ...
  // Inject css
  if (this.css) {
    elastoCss(this.elementName, this.css, this.useNative)
  }
...
Elastomer.prototype._bindView = function () {
  ...
  this._postPreLink = (function () {
    ...
    if (host.link) {
      this._unlink = host.link(this.getScope(), this)
    }
  ...
  function injectLightDom () {
    var content = self.host.querySelector('content')
    if (!content) {
      return
    }
    content.setAttribute(self.prefix + '-ignore', '')
    lightDom.forEach(function (el) {
      console.log(el)
