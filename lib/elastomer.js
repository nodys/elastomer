...
Elastomer.prototype.createdCallback = function () {
  ...
  if (this.css) {
    if (this.css.onChange) {
      this.css.onChange(this._cssUpdate.bind(this))
    }
  }
...
Elastomer.prototype._cssUpdate = function (css) {
  console.log('Css udpate')
}

Elastomer.prototype._shimCss = function (css) {
  // /([^\r\n,{}]+)(,(?=[^}]*{)|\s*{)/g
  return css
}
  ...
  self._postPreLink = function () {
    ...
    if (self.html) {
      if (self.css && (typeof (self.css) !== 'function')) {
        shadow.innerHTML = self.html.toString() + '<style>' + self._shimCss(self.css) + '</style>'
      } else {
