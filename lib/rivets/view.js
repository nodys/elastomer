...
export default class View {
  ...
  build() {
    ...
    // Refactoring: prevent re-optimization of the parse function

    for (let i = 0, len = this.els.length; i < len; i++) {
      parse(this.els[i], this)
    }
...
// Refactoring: This function cost a lot, and should not be
// re-optimized at each calls as before
function parse (node, self) {
  let block = false

  if (node.nodeType === 3) {
    let delimiters = self.templateDelimiters
      ...
      if (tokens.length) {
        if (!(tokens.length === 1 && tokens[0].type === 0)) {
          tokens.forEach((token, i) => {
            ...
            if (token.type === 1) {
              self.buildBinding(TextBinding, text, null, token.value)
            }
          })
    ...
    block = self.traverse(node)
  }

  if (!block) {
    Array.prototype.slice.call(node.childNodes).forEach(function (node) { parse(node, self) })
  }
}
